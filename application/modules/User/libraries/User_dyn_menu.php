...
class User_dyn_menu {
    ...
    function build_menu($table = 'tb_dyn_menu')
    {
        ...
        $table = $this->ci->user_model->get_table_levels_user($level_user_id);
        $results = $this->ci->user_model->get_navmenu_userdata($table,$level_user_id);
    ...
    {
    	$results_ref = $this->ci->user_model->get_childs_navmenu_userdata($table,$ref_id);
        $has_subcats = FALSE;
        $html_out = '<ul>';
    ...
    function get_opengraph_content($table,$menu_url,$id)
    {
		...
		{
		foreach ($results as $row) {
		$property['file'] = upload_url().''.$menu_url.'/123/'.decrypt_ciphertext($row->file);
