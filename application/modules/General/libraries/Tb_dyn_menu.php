...
class Tb_dyn_menu {
     ...
     */
    function build_navbar_errors($table = 'tb_dyn_menu')
    {
        $menu = array();
        // use active record database to get the menu.
        $limit = 0;
        $start = 0;
        $results = $this->ci->menu_model->get_navmenu_data($table,$limit,$start);
    	if (is_array($results) || is_object($results))
		{
		$html_out  = "\t".'<header id="lm-header" class="lm-header lm-hd lm-navbar-wrapper">'."\n";
    	...
    	$html_out .= "\t\t".'<nav class="lm-primary-nav lm-nav-top lm-hd-nav lm-nav" id="lm-primary-nav"><ul class="lm-nav-ul">'."\n";
			...
			// CodeIgniter's anchor(uri segments, text, attributes) tag.
                ...
                if($position == 1 && !empty($icon))
                {
                    if ($id == 531031)
                    {
					$html_out .= "\t\t\t\t".'<li class="lm-nav-li" title="'.$title.'"><a href="'.$url.'" '.$attribute.'><span class="'.$icon.'"></span></a></li>';
        			}
        			}
				}
			}
    ...
    function build_menu($table = 'tb_dyn_menu')
    {
		...
		if($this->login){
		$html_out .= "\t\t\t\t".'<li class="lm-navmenu-link" title="dashboard">'.anchor('dashboard', '<span>dashboard</span>');
		}else{
		$html_out .= "\t\t\t\t".'<li class="lm-navmenu-link" title="home">'.anchor('home', '<span>home</span>');	
		}
        ...
        $html_out .= "\t\t".'</ul>' . "\n";
        $html_out .= "\t".'</div>' . "\n";
		$html_out .= $this->build_getintouch();
		$html_out .= $this->ci->tb_dyn_medsos->build_medsos();
		$html_out .= "\t".'</nav>' . "\n";
        return $html_out;
    }
    ...
    function build_getintouch($table = 'tb_dyn_menu')
    {
		$results = $this->ci->menu_model->get_navmenu_data($table);
        $html_out  = "\t".'<div class="lm-sidebar-row">'."\n";
		$html_out .= "\t\t".'<ul class="lm-navmenu-right-ul">'."\n";
        ...
        if (is_array($results) || is_object($results))
		{
            ...
            if ($dyn_group_id == 532702 && $parent == 0)    // are we allowed to see this menu?
                ...
                if($position == 2)
                {
                    ...
                    {
                        // CodeIgniter's anchor(uri segments, text, attributes) tag.
                        $html_out .= "\t\t\t\t".'<li class="lm-button lm-touch-link lm-touch" id="lm-touch-link" title="'.$title.'">'.anchor($url, '<span>'.ucfirst($title).'</span>').'</li>';
                    }
                }
              }
              }
    ...
    function build_menu_title($table = 'tb_dyn_menu')
    {
		...
		$url = strtolower($url);
            if($url == uri_string()) {
                ...
                $html_out .= '<h1 class="reset"><span id="typed">'.ucfirst($page_title).'</span></h1>';
    ...
    function get_opengraph_content($table,$menu_url,$id)
    {
    	$property = null;
    	$this->ci->load->model(array('content_model'));
        $results = $this->ci->content_model->get_content_opengraph($table,$menu_url,$id);
        if (is_array($results) || is_object($results))
		{
		foreach ($results as $row) {
		$property['file'] = upload_url().''.$menu_url.'/'.decrypt_ciphertext($row->id).'/'.decrypt_ciphertext($row->file);
		$property['caption'] = decrypt_ciphertext($row->caption);
		$property['title'] = decrypt_ciphertext($row->title);
		$property['created_on'] = decrypt_ciphertext($row->created_on);
		$property['title'] = ucwords($property['title']);
		}
		}
        return $property;
